...
public class Game {
    ...
    private static final int RED_MOVES = 2;
    private static final int EMPTY_SPACE = 0;
    private static final int BLUE_MARKER = 4;
    private static final int RED_MARKER = 5;
    ...
    public boolean legalMove(int To, int From, int color) {
        Log.i("GameLogic","Color: " + Integer.toString(color));
        Log.i("GameLogic", "Turn: " + Integer.toString(turn));
        if (color == turn) {
            ...
            Log.i("GameLogic", "no one moves");
            return false;
        }
        // will never get here :/
        Log.i("GameLogic", "got here somehow");
        return false;
    }
    ...
    /**
     * Returns true if the selected player have less than three
     * markerss left.
     */
    public boolean win(int color) {
        ...
        int count = 0;
        while (count < 23) {
            if (gameplan[count] != EMPTY_SPACE &&
                    gameplan[count] != color)
                countMarker++;
            count++;
        }
